package caa4444.ashcollector.nodes;

import caa4444.ashcollector.misc.Const;
import caa4444.ashcollector.misc.Methods;
...
public class MakeLeather extends Node {
    ...
    public void execute() {
        ...
        if (Widgets.get(1371).validate()) {
            if (Widgets.get(1371, 44).getChild(Variables.i.getFIRE_ID()).getTextureId() == -1) {
                Widgets.get(1371, 44).getChild(Variables.i.getFIRE_ID()).click(true);
            }
            Methods.s("Crafting Leather");
            if (Widgets.get(1371, 44).getChild(Variables.i.getFIRE_ID()).getTextureId() == 15201 &&
